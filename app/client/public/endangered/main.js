...
function extractPopulationNames(ipac) {
    var names = ""
    for( var el in ipac.resources.populationsBySid ) {
        if( obj.hasOwnProperty( el ) ) {
          names += "<div>obj[el]</div>";
    }}
    return names
}

async function makeMarkers(map, item, nodes) {
    const green = '#50C878'
    ...
    let noEndangeredSpecies = isEmpty(data.ipac.resources.populationsBySid)
...
async function updateWaterways(event) {
    ...
    for (const item of data) {
        ...
        if (item["type"] === "node") {
            ...
            makeMarkers(event.target, item, nodes)
        }
    }
}
